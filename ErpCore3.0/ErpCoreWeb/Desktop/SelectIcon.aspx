<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SelectIcon.aspx.cs" Inherits="Desktop_SelectIcon" %>
...
// <!CDATA[
        ...
        function onSelectIcon(src, filename) {
            parent.onSelectIcon(src, filename);
            parent.$.ligerDialog.close();
        }
// ]]>
    </script>
...
<body>
    ...
    <% 
        string sPath = string.Format("../{0}/MenuIcon/", Global.GetDesktopIconPathName());
        DirectoryInfo di = new DirectoryInfo(Server.MapPath(sPath));
        FileInfo[] fis= di.GetFiles();
        foreach (FileInfo fi in fis)
        {
            string src = string.Format("../{0}/MenuIcon/{1}", Global.GetDesktopIconPathName(), fi.Name);
            %>
            <div style="float:left"><img src="<%=src %>" height="64" width="64" onclick="onSelectIcon('<%=src %>','<%=fi.Name%>')" /></div>
            <%
        }
        %>
    </div>

...
public partial class View_EditDetailRecord2 : System.Web.UI.Page
{
    ...
    void PostData()
    {
        ...
        Response.Write("<script>parent.parent.onOkEditDetailRecord2();</script>");
    }
    //验证数据
    bool ValidateData()
    {
        ...
        foreach (CBaseObject objCol in m_Table.ColumnMgr.GetList())
        {
            ...
            if (col.ColType == ColumnType.string_type)
            {
                ...
                {
                    RegisterStartupScript("starup", string.Format("<script>alert('{0}长度不能超过{1}！');</script>", col.Name, col.ColLen));
                    return false;
                }
            }
            else if (col.ColType == ColumnType.datetime_type)

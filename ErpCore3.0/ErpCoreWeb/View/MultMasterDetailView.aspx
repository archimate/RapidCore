<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MultMasterDetailView.aspx.cs" Inherits="View_MultMasterDetailView" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        //刷新
        function onRefresh() {
            var url = 'MultMasterDetailView.aspx?Action=GetData&vid=<%=Request["vid"] %>&ParentId=<%=Request["ParentId"] %>';
            grid.set({ url: url });
            //grid.loadData();
        }
        //过滤
        function onFilter() {
            ...
            buttons: [
                { text: '确定', onclick: function(item, dialog) {
                    $.ligerDialog.close();
                    var url = 'MultMasterDetailView.aspx?Action=GetData&vid=<%=Request["vid"] %>&ParentId=<%=Request["ParentId"] %>&Filter=1';
                    grid.set({ url: url });
                    //grid.loadData();
                }
                },
        ...
        function onAdd() {
            $.ligerDialog.open({ title: '新建', url: 'AddMultMasterDetailViewRecord.aspx?vid=<%=Request["vid"] %>&ParentId=<%=Request["ParentId"] %>', name: 'winAddRec', height: winHeight, width: winWidth, showMax: true, showToggle: true, isResize: true, modal: false, slide: false, 
            buttons: [
                { text: '确定', onclick: function(item, dialog) {
        ...
        function onOkAddMultMasterDetailViewRecord(){
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onCancelAddMultMasterDetailViewRecord(){
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onEdit() {
            ...
            $.ligerDialog.open({ title: '修改', url: 'EditMultMasterDetailViewRecord.aspx?vid=<%=Request["vid"] %>&ParentId=<%=Request["ParentId"] %>&id=' + row.id, name: 'winEditRec', height: winHeight, width: winWidth, showMax: true, showToggle: true, isResize: true, modal: false, slide: false, 
            buttons: [
                { text: '确定', onclick: function(item, dialog) {
        ...
        function onOkEditMultMasterDetailViewRecord(){
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onCancelEditMultMasterDetailViewRecord(){
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onDelete() {
    ...
    <script type="text/javascript">
        ...
        $(function ()
        {
            ...
            columns: [
                ...
                url: 'MultMasterDetailView.aspx?Action=GetData&vid=<%=Request["vid"] %>&ParentId=<%=Request["ParentId"] %>',
                dataAction: 'server', pageSize: 30,
                width: '100%', height: '50%',
                onSelectRow: function (data, rowindex, rowobj)
                {
                    var url = 'MultDetailList.aspx?vid=<%=Request["vid"] %>&ParentId=<%=Request["ParentId"] %>&pid=' + data.id;
                    var arr = tab.getTabidList();
                    for (var i = 0; i < arr.length; i++) {
                        tab.overrideTabItem(arr[i],{url:url+'&vdid='+arr[i]});
                    }
                }
            });
        });
        ...
        {                
            ...
            <%foreach(CBaseObject obj in m_View.ViewDetailMgr.GetList())
            {
                ...
                %>
                f_addTab('<%=vd.Id %>','<%=vd.Name %>','MultDetailList.aspx?vid=<%=Request["vid"] %>&vdid=<%=vd.Id %>&ParentId=<%=Request["ParentId"] %>');
                <%
            }
             %>
        });

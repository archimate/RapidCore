<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MultDetailListRecord2.aspx.cs" Inherits="View_MultDetailListRecord2" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function onAdd() {
            $.ligerDialog.open({ title: '新建', url: 'AddMultDetailRecord2.aspx?vid=<%=Request["vid"] %>&vdid=<%=Request["vdid"] %>', name: 'winAddRec', height: winHeight, width: winWidth, showMax: true, showToggle: true,  isResize: true, modal: false, slide: false, 
            buttons: [
        ...
        function onOkAddMultDetailRecord2() {
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onCancelAddMultDetailRecord2() {
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onEdit() {
            ...
            $.ligerDialog.open({ title: '修改', url: 'EditMultDetailRecord2.aspx?vid=<%=Request["vid"] %>&vdid=<%=Request["vdid"] %>&id=' + row.id, name: 'winEditRec', height: winHeight, width: winWidth, showMax: true, showToggle: true, isResize: true, modal: false, slide: false, 
            buttons: [
                { text: '确定', onclick: function(item, dialog) {
        ...
        function onOkEditMultDetailRecord2() {
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onCancelEditMultDetailRecord2() {
            grid.loadData(true);
            $.ligerDialog.close();
        }
        function onDelete() {
    ...
    <script type="text/javascript">
        ...
        $(function ()
        {
            ...
            columns: [
                ...
                url: 'MultDetailListRecord2.aspx?Action=GetData&vid=<%=Request["vid"] %>&vdid=<%=Request["vdid"] %>&pid=<%=Request["pid"] %>',
                dataAction: 'server', 
                usePager: false,

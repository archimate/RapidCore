<%@ Page Language="C#" AutoEventWireup="true" CodeFile="WorkflowDefPanel.aspx.cs" Inherits="Workflow_WorkflowDefPanel" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function onAdd() {
            $.ligerDialog.open({ title: '新建', url: 'AddWorkflowDef.aspx?B_Company_id=<%=Request["B_Company_id"] %>&catalog_id=<%=Request["catalog_id"] %>', name: 'winAddRec', height: 450, width: 600, showMax: true, showToggle: true, isResize: true, modal: false, slide: false, 
            buttons: [
        ...
        function onEdit() {
            ...
            $.ligerDialog.open({ title: '修改', url: 'EditWorkflowDef.aspx?id=' + row.id + '&B_Company_id=<%=Request["B_Company_id"] %>&catalog_id=<%=Request["catalog_id"] %>', name: 'winEditRec', height: 450, width: 600, showMax: true, showToggle: true, isResize: true, modal: false, slide: false, 
            buttons: [
                { text: '确定', onclick: function(item, dialog) {
            ...
            columns: [
                ...
                for (int i=0;i<lstObjC.Count;i++)
                {
                    ...
                    if (col.Code.Equals("id", StringComparison.OrdinalIgnoreCase))
                        ...
                        continue;
                    else if (col.Code.Equals("B_Company_id", StringComparison.OrdinalIgnoreCase))
                        continue;
                ...
                url: 'WorkflowDefPanel.aspx?Action=GetData&catalog_id=<%=Request["catalog_id"] %>&B_Company_id=<%=Request["B_Company_id"] %>',

<%@ Page Language="C#" AutoEventWireup="true" CodeFile="EditLink.aspx.cs" Inherits="Workflow_EditLink" %>
        ...
        <input name="WF_WorkflowDef_id" type="hidden" id="WF_WorkflowDef_id" value="<%=Request["wfid"] %>"/>
            ...
            <tr>
                ...
                <select name="Result" id="Result" ltype="select">
                  <option value="1" <%if(m_BaseObject.Result== enumApprovalResult.Accept){ %>selected<%} %>>接受</option>
                  <option value="2" <%if(m_BaseObject.Result== enumApprovalResult.Reject){ %>selected<%} %>>拒绝</option>
                  </select>
                </td>
                <td align="left"></td>
            </tr>
            <tr>
                ...
                <table>
                <tr><td>
                <textarea cols="100" rows="4" class="l-textarea" id="Condiction" style="width:400px"><%=m_BaseObject.Condiction%></textarea>
                </td></tr>
                <tr><td>
                  ...
                  <%                      
                      ...
                      foreach (CBaseObject obj in lstAD)
                      {
                          ...
                          if (ActivesDef.WType == ActivesType.Start)
                              continue;
                          %>
                  <option value="<%=ActivesDef.Id%>" <%if(m_BaseObject.NextActives==ActivesDef.Id){ %>selected<%} %>><%=ActivesDef.Name%></option>
                          <%
                      }
                       %>
                  </select>

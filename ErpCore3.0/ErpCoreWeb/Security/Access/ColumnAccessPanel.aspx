<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ColumnAccessPanel.aspx.cs" Inherits="Security_Access_ColumnAccessPanel" %>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            grid = $("#gridTable").ligerGrid({
                ...
                url: 'ColumnAccessPanel.aspx?Action=GetData&B_Company_id=<%=Request["B_Company_id"] %>',
                dataAction: 'server',
                usePager: false,
                enabledEdit: true,
                rownumbers: true,
        ...
        function ReloadGrid()
        {
            ...
            if (document.getElementById("cbUType").selectedIndex == 2)
                utype = "1";
            var url = 'ColumnAccessPanel.aspx?Action=GetData&B_Company_id=<%=Request["B_Company_id"] %>&UType=' + utype + '&Uid=' + $("#hidUid").val() + '&FW_Table_id=' + $("#cbTable").val();
            grid.set({ url: url });
            grid.loadData();
        }
        ...
        function btSave_onclick() {
            ...
            if (document.getElementById("cbTable").selectedIndex < 0) {
            ...
            $.post(
                'ColumnAccessPanel.aspx',
                {
                    Action: 'PostData',
                    B_Company_id: '<%=Request["B_Company_id"] %>',
                    UType: utype,
                    Uid: $("#hidUid").val(),
                    FW_Table_id: $("#cbTable").val(),
                    postData: postData
                },
                 function(data) {
                     if (data == "" || data == null) {
...
<form id="from1">
    <table cellpadding="0" cellspacing="0" class="l-table-edit" >
        <tr>
            ...
            <td align="right" class="l-table-edit-td">表:</td>
            <td align="left" class="l-table-edit-td">
                <select id="cbTable" runat="server" onchange="return cbTable_onchange()">
                    <option></option>
                </select>
            </td>

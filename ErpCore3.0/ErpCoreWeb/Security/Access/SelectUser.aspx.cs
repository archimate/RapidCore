...
public partial class Security_Access_SelectUser : System.Web.UI.Page
{
    ...
    void GetData()
    {
        ...
        {
            CUser user = (CUser)lstObj[i];

            string sOrgName = "";
            List<COrg> lstOrg = m_Company.OrgMgr.FindByUser(user.Id);
            foreach (COrg Org in lstOrg)
            {
                sOrgName += Org.Name + ";";
            }
            sData += string.Format("{{ \"id\": \"{0}\",\"Name\":\"{1}\", \"OrgName\":\"{2}\" }},"
                , user.Id, user.Name,  sOrgName);

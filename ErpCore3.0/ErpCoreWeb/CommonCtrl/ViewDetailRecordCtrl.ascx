<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ViewDetailRecordCtrl.ascx.cs" Inherits="CommonCtrl_ViewDetailRecordCtrl" %>
...
<% 
    int iUICol = 0;
    //foreach (CColumnInViewDetail civd in m_ViewDetail.ColumnInViewDetailMgr.GetList())
    foreach (CBaseObject objCol in m_Table.ColumnMgr.GetList())
    {
        ...
        //字段默认值
        CColumnDefaultValInViewDetail cdvivd = m_ViewDetail.ColumnDefaultValInViewDetailMgr.FindByColumn(col.Id);
        if (cdvivd!=null && cdvivd.ReadOnly == true)
            bReadOnly = true;
        ...
        if (m_BaseObject == null)
        {
            if (bHideColumn)
            {%>
                <input name="_<%=col.Code %>" type="hidden" id="_<%=col.Code %>" value="<%=GetColumnDefaultVal(cdvivd,col) %>" />
                <%
                continue;
            }
        %>
    <tr>
        ...
        <%if (col.ColType == ColumnType.string_type)
          { %>
        <input name="_<%=col.Code %>" type="text" id="_<%=col.Code %>" ltype="text" value="<%=GetColumnDefaultVal(cdvivd, col) %>"  <%if(bReadOnly){ %>readonly="readonly"<%} %> />
        <%}
        ...
        <%}
          ...
          { %>
          <input name="_<%=col.Code %>" type="text" id="_<%=col.Code %>" value="<%=GetColumnDefaultVal(cdvivd, col) %>" ltype="text"   <%if(bReadOnly){ %>readonly="readonly"<%} %>/>
          <%}
          ...
          { 
              string defval = GetColumnDefaultVal(cdvivd, col).ToLower();
              %>
          ...
          <script type="text/javascript">              
          $(function() {
          $("#_<%=col.Code %>").ligerDateEditor({ initValue: '<%=GetColumnDefaultVal(cdvivd, col) %>', showTime: <%=GetShowTimel(col) %> });
          });
          </script>
          <%}
          else if (col.ColType == ColumnType.ref_type)
          {
              if (bReadOnly)
              {%>
            <input name="Ref_<%=col.Code %>" type="text" id="Ref_<%=col.Code %>" value="<%=GetColumnRefDefaultVal(cdvivd,col) %>" ltype="text" readonly="readonly" />
            <input name="_<%=col.Code %>" type="hidden" id="_<%=col.Code %>" value="<%=GetColumnDefaultVal(cdvivd,col) %>"  />
            <%}
            else if (bPopupSelDialog)
            {%>
            <input name="Ref_<%=col.Code %>" type="text" id="Ref_<%=col.Code %>" value="<%=GetColumnRefDefaultVal(cdvivd,col) %>" ltype="text"/>
            <input name="_<%=col.Code %>" type="hidden" id="_<%=col.Code %>" value="<%=GetColumnDefaultVal(cdvivd,col) %>" />
            <%}
              else
              { %>
          ...
          else if (col.ColType == ColumnType.enum_type)
          {
              if (bReadOnly)
              {%>
            <input name="_<%=col.Code %>" type="text" id="_<%=col.Code %>"  value="<%=GetColumnDefaultVal(cdvivd, col) %>" ltype="text" readonly="readonly" />
            <%}
              else
              { %>
          <select name="_<%=col.Code %>" id="_<%=col.Code %>" ltype="select"  >
          <option value="">(空)</option>
                <%//引用显示字段优先
                  if (col.RefShowCol != Guid.Empty)
                  {
                      ...
                      foreach (CBaseObject objRef in lstObjRef)
                      {
                          bool bIsCurrent = GetColumnDefaultVal(cdvivd, col).Equals(objRef.GetColValue(RefShowCol).ToString(), StringComparison.OrdinalIgnoreCase);
                      %>
                  ...
                  else
                  {
                      ...
                      foreach (CBaseObject objEV in lstObjEV)
                      {
                         ...
                         %>
                         <option value="<%=cev.Val%>" <%if(GetColumnDefaultVal(cdvivd, col)==cev.Val){ %>selected<%} %>><%=cev.Val%></option>
                      <%}
                } %>
        </select>
          <%}

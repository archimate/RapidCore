...
namespace ErpCoreModel.Invoicing
{
    ...
    public class CSaleReturnNoteDetail : CBaseObject
    {

        public CSaleReturnNoteDetail()
        {
            TbCode = "XS_SaleReturnNoteDetail";
            ClassName = "ErpCoreModel.Invoicing.CSaleReturnNoteDetail";
        ...
        public Guid XS_SaleReturnNote_id
        {
            get
            {
                if (m_arrNewVal.ContainsKey("xs_salereturnnote_id"))
                    return m_arrNewVal["xs_salereturnnote_id"].GuidVal;
                else
                    return Guid.Empty;
            }
            set
            {        
                if (m_arrNewVal.ContainsKey("xs_salereturnnote_id"))
                    m_arrNewVal["xs_salereturnnote_id"].GuidVal = value;
                else
                {
                    CValue val = new CValue();
                    val.GuidVal = value;
                    m_arrNewVal.Add("xs_salereturnnote_id", val);
                }
            }
        }
    }
